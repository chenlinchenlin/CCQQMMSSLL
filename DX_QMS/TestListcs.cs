...
namespace DX_QMS
{
    public partial class TestListcs : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        string datareceive(string str)
        {
            string[] sArray = str.Split(new char[2] { ',', '，' });
            if (sArray.Length != 3)
                return "";
            return sArray[1].TrimStart('+');
        }
        private string ChangeToDecimal(string strData, long Multiple)
        {
            string Flat = "";
            Decimal dData = 0.0M;
            if (strData.Contains("e"))
            ...
            if (Flat == "")
                return (Math.Round(dData * Multiple, 3)).ToString();
            else
                return Flat + (Math.Round(dData * Multiple, 3)).ToString();
        }
        private void updateSmallBoxInfo(string text)
        {
            string[] sArray = text.Split(new char[2] { ',', '，' });
            if (text.Contains(",") && sArray.Length != 3)
            {
                try
                {
                    string s = text.Substring(0, text.IndexOf(","));
                    string tval1 = ChangeDataToD(s).ToString();

                    lblTestValueShow.Text = tval1;
                    txttestvalue.Text = tval1;

                    float n = 0;
                    if (float.TryParse(tval1, out n))
                    {
                        if (float.Parse(tval1) <= 0)
                        {
                        ...
                        //测试值在范围之外,不处理
                        if (float.Parse(tval1) < lowlittle)
                        {
                            lblTestValueShow.ForeColor = Color.Red;
                            return;
                        }
                        if (float.Parse(tval1) > upbig)
                        {
                            lblTestValueShow.ForeColor = Color.Red;
                            return;
                        }
                        ...
                        //在测试值范围内,则进行记录测试值
                        if ((float.Parse(tval1) >= float.Parse(txtscope1.Text == "" ? "0" : txtscope1.Text)) && (float.Parse(tval1) <= float.Parse(txtscope2.Text == "" ? "0" : txtscope2.Text)))
                        {
                            lblTestValueShow.ForeColor = Color.Green;
                            //每次测试时只能有一个队列数数据(valQueue测试值OK队列,valQueue2测试值NG队列),为了保证连续5次数据都满足
                            valQueue.Enqueue(float.Parse(tval1));
                            valQueue2.Clear();
                            //为了记录当测试数据一直不满足5个连续值时,当测试超过了10次,系统就提示"测试值测不好",从而结束此次测试
                            testcount++;
                        ...
                        else if ((float.Parse(tval1) >= lowlittle && float.Parse(tval1) <= float.Parse(txtscope1.Text == "" ? "0" : txtscope1.Text))
                                  || (float.Parse(tval1) >= float.Parse(txtscope2.Text == "" ? "0" : txtscope2.Text) && float.Parse(tval1) <= upbig))
                        {
                            lblTestValueShow.ForeColor = Color.YellowGreen;
                            valQueue2.Enqueue(float.Parse(tval1));
                            valQueue.Clear();
                            testcount++;
                            ...
                            if (valQueue2.Count == 5)
                            {
                                ...
                                //在测试值范围之内,系统弹出一对话框,停留5秒后再写入系统,以免连续写入多笔数据
                                StartKiller();
                                if (MessageBox.Show("测试值:" + txttestvalue.Text + ",检验结果:" + cbNG.Text, "MessageBox", MessageBoxButtons.OK, MessageBoxIcon.Warning) == DialogResult.Cancel)
                                {
                                    return;
                                }
                                //如果项目未测试完(即"确定"可按)
            ...
            else if (text.Contains(",") && sArray.Length == 3)
            {
                try
                {
                    string s = datareceive(text);
                    // string tval1 = ChangeDataToD(s).ToString();
                    string tval1 = "";

                    if ((txttestsubitem.Text == "容值测试") && lblprodinfo.Text.ToUpper().Contains("PF"))
                    {
                        lblunit.Text = "pF";
                        tval1 = ChangeToDecimal(s, 1000000000000);
                    }
                    else if ((txttestsubitem.Text == "容值测试") && lblprodinfo.Text.ToUpper().Contains("UF"))
                    {
                        lblunit.Text = "uF";
                        tval1 = ChangeToDecimal(s, 1000000);
                    }
                    else if ((txttestsubitem.Text == "容值测试") && lblprodinfo.Text.ToUpper().Contains("NF"))
                    {
                        lblunit.Text = "nF";
                        tval1 = ChangeToDecimal(s, 1000000000);
                    }
            ...
            //读出串口上的数据
            tval = text.Substring(14, 6).Trim();
            if (txtlotno.Text.Trim() == "") return;

            float n1 = 0;
            if (float.TryParse(tval, out n1))
            {
                ...
                if (float.Parse(tval) < lowlittle)
                {
                    ...
                    //if (valQueue.Count == 5 && lblunit.Text==lblsetunit.Text)
                    if (valQueue.Count == 5)
                    {
                        txttestvalue.Text = valQueue.Last().ToString();
                        cbOK.Checked = false;
        ...
        private void TestListbind(string testtype, string testitem, string testsubitem, string packtype, string lotno, string sampleqty)
        {
            ...
            if (int.Parse(txtsampleqty.Text == "" ? "0" : txtsampleqty.Text ) <= int.Parse(txtsamplefactqty.Text == "" ? "0" : txtsamplefactqty.Text) && testsubitem.Contains("外观"))
            {
            ...
            else if (int.Parse(txtsampleqty.Text == "" ? "0" : txtsampleqty.Text) <= int.Parse(txtsamplefactqty.Text == "" ? "0" : txtsamplefactqty.Text))
            {
              txttestvalue.Enabled = false;
                txtremarks.Enabled = false;
                btnOK.Enabled = false;
        ...
        private void bindSubitemInfo()
        {
            ...
            //if (txttestsubitem.Text == "型号核对")
            //{
            //    btnOK.Enabled = false;
            //    TestSamplecheck checksample = new TestSamplecheck(sproductcode);
            //    checksample.ShowDialog();
            //    string sample = checksample.ifcorrect;
            //    if (sample != "虚拟编码正确")
            //    {
            //        btnOK.Enabled = false;
            //        return;
        ...
        {
           ///////// txttestsubitem_SelectedIndexChanged(sender,e);
        }


        bool  checktestitem(string materialcode)
        {   
            string sql = @"    select  distinct  TestSubItem  from IQC_TestProgSet where  TestItem = '可靠性' and Productcode = '"+ materialcode + "'";
            DataTable dt = DbAccess.SelectBySql(sql).Tables[0];
            ...
            else
            {
                ...
                {
                    if (dt.Rows[i]["TestSubItem"].ToString().Contains("切片测试"))
                    {
                        return true;
                    }
        ...
        private void getInfo(string lotno)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["vendorname"].ToString().Contains("天河星供应链有限公司") && ds.Tables[0].Rows[0]["materialcode"].ToString().StartsWith("5002"))
                {
                    if (!checktestitem(ds.Tables[0].Rows[0]["materialcode"].ToString()))
                    {
                        lblinfo.Text = "该供应商没有维护切片测试";
                        lblinfo.ForeColor = Color.Red;
                        MessageBox.Show("该供应商没有维护切片测试", "提醒",MessageBoxButtons.OK ,MessageBoxIcon.Information);
                        return;
                    }
                }
        ...
        private void showpic(string pcode)
        {
            ...
            if (Connect(serverFilePath))
            {
                ...
                else
                {
                    ...
                    if (dtold.Rows.Count > 0)
                    {
                        ...
                        if (Directory.Exists(floerPathold))
                        {
                            ...
                            pt = pt.Where(a => !a.EndsWith("Thumbs.db")).ToArray();
                            if (pt.Length == 0) return;
                            try
                            {
                                if (pt.Length == 1)
                ...
                string ssql = "select PNo, PName, userid, eventtime, sup,ISNULL(PackType,'') PackType,checklist, Remarks from MaterialBitPIC where PNo='" + pcode + "' order  by eventtime asc";
                DataSet ds = Common.DbAccess.SelectBySql(ssql);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    int i = ds.Tables[0].Rows.Count;
                    string show = "";
                    lblpicdes.Text = "品牌商:"+ds.Tables[0].Rows[0]["sup"].ToString() +" ,包装方式："+ds.Tables[0].Rows[0]["PackType"].ToString()+ " ,检查项:" + ds.Tables[0].Rows[0]["checklist"].ToString() + " ,备注:" + ds.Tables[0].Rows[0]["Remarks"].ToString();
                    for (int j = 1; j < i; j++)
                    {
                        show =show + " 【"+ds.Tables[0].Rows[j]["sup"].ToString()+"；"+ds.Tables[0].Rows[j]["PackType"].ToString()+ "；" + ds.Tables[0].Rows[j]["checklist"].ToString()+"；"+ds.Tables[0].Rows[j]["Remarks"].ToString() +"】 ";
                    }
        ...
        {
            string ifhavepostion = "";
            IQC ic = new IQC();
            DataSet ds = ic.SelectTestSamplePosition("查询", pcode, "", "", "", "", "", "", "", "", "", "", "", Login.username, "");
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                if (pcode.StartsWith("5"))
                {
                    string sql = @"  select TestType from IQC_TestProgSet where Productcode = '" + pcode + "' group by TestType  ";
                    string strtesttype = "";
                    DataTable dt = DbAccess.SelectBySql(sql).Tables[0];
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        {
                            strtesttype += dt.Rows[n]["TestType"].ToString();
                        }

                        if (strtesttype.Contains("变压器（贴片）") || strtesttype.Contains("保险丝") || strtesttype.Contains("发光二极管") || strtesttype.Contains("滤波器（贴片）") || strtesttype.Contains("贴片连接器") || strtesttype.Contains("模块"))
                        {
                            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                            {
                                ifhavepostion += ds.Tables[0].Rows[i]["存放位置"].ToString();
                            }
                            if (ifhavepostion == "")
                            {
                                MessageBox.Show("该料号没有样品位置", "提醒", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                return;
                            }
                        }
                    }
            ...
            else
            {
                if (pcode.StartsWith("5"))
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (strtesttype.Contains("变压器（贴片）") || strtesttype.Contains("保险丝") || strtesttype.Contains("发光二极管") || strtesttype.Contains("滤波器（贴片）") || strtesttype.Contains("贴片连接器") || strtesttype.Contains("模块"))
                        {
                            MessageBox.Show("该料号没有维护样品位置", "提醒", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                    }
                }
        ...
        string MaterialSource( string Materialcode,string supplier)
        {
            string materialsource = ""; 
            string sql = " select materialsource from IQC_ProxyCardConfig  where productcode = '" + Materialcode + "' and supplier like '%"+supplier+ "%' ";
            DataTable  dt = Common.DbAccess.SelectBySql(sql).Tables[0];
        ...
        bool checkSamplepostion(string productcode)
        {
            bool flag = false;
            string sql = "  select sampleClass from IQC_TestSamplePosition  where productcode = '"+productcode+"'   ";
            DataTable dt = DbAccess.SelectBySql(sql).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i=0; i<dt.Rows.Count ; i++)
                {
                    if (dt.Rows[i]["sampleClass"].ToString() == "实物样")
                    {
                        return true;
                    }
                }
        ...
        string MouldLife(string productcode)
        {
            string mouldmessage = "";
            string sql = " select Mouldcode,Mouldtype,ColorMode,MouldQty,MouldLife,RestQty from IQC_MouldCycle where materialcode = '"+productcode+ "'  ";
            DataTable dt = DbAccess.SelectBySql(sql).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                mouldmessage = "模具为："+ dt.Rows[0]["MouldQty"].ToString()+ "；类型为："+ dt.Rows[0]["Mouldtype"].ToString()+"；寿命为："+ dt.Rows[0]["MouldLife"].ToString()+"；剩余次数为："+ dt.Rows[0]["RestQty"].ToString();

                int MouldLife = int.Parse(dt.Rows[0]["MouldLife"].ToString());
                int RestQty = int.Parse(dt.Rows[0]["RestQty"].ToString());
                if (RestQty< MouldLife*0.2 && RestQty > MouldLife * 0.1)
                {
                    lblinfoinstock.ForeColor = Color.Yellow;
                }
                else if (RestQty < MouldLife * 0.1)
                {
        ...
        {
            string productname = "";
            string Oraclesql = "  select description from  cux_item_material_v where segment1 = '"+productcode+ "' ";
            DataTable dt  = DbAccess.SelectByOracle(Oraclesql).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        private void txtlotno_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                ...
                //////////////getInfoNew(txtlotno.Text.Trim());  

                string ifflag = lblinfo.Text;
                if (ifflag.Contains("没有维护切片测试") || ifflag.Contains("没有做接收或转移") || ifflag.Contains("没有生成批次") || ifflag.Contains("批次号错误或不存在"))
                    return;
                ...
                if (checkSamplepostion(ds.Tables[0].Rows[0]["materialcode"].ToString()))
                {
                    TestSamplecheck checksample = new TestSamplecheck(ds.Tables[0].Rows[0]["materialcode"].ToString());
                    checksample.ShowDialog();
                    string sample = checksample.ifcorrect;
                    if (sample != "虚拟编码正确")
                    {
                ...
                {                
                    ...
                    if (txttotalqty.Text != "")
                    {
                        lotqty = int.Parse(txttotalqty.Text);
                    }
                    else
                    {
                    ...
                    if (ds.Tables[0].Rows[0]["materialcode"].ToString() != sproductcode)
                    {
                        ...
                        // txtremarks.Text = txtremarks.Text + ":" + s;
                        ...
                        {
                           // MessageBox.Show(spcode + "物料处于:" + s + ",请注意！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);

                            //string sqlAQL = @"select AQLValue from IQC_TestProgSet where Productcode ='" + spcode + "'";
                            //DataTable ddt = DbAccess.SelectBySql(sqlAQL).Tables[0];
                            //string AQLValue = ddt.Rows[0]["AQLValue"].ToString();

                            //string txtCode = @"Select s.Code from IQC_TestSTD105ECode c ";
                            //txtCode += "  inner join ";
                            //txtCode += " (Select " + lotqty + " lotfactqty,AQLValue,AQL,CheckLevel,Ac,Re from IQC_TestSTD105ECheckSet i inner join IHPS_QUALITY_SPC_AQLIS02859 s on i.Code=s.Code  where LotSizemin<=" + lotqty.ToString() + " and LotSizemax>=" + lotqty.ToString() + " and CheckLevel='II') s on c.Code=s.Code";
                            //DataTable dds = DbAccess.SelectBySql(txtCode).Tables[0];
                            //string Code = dds.Rows[0]["Code"].ToString();
                            ...
                            //textBox_Ac.Text = dt.Rows[0]["Ac"].ToString();
                            //textBox_Re.Text = dt.Rows[0]["Re"].ToString();

                        }
                        else if (s == "放宽检验")
                        {
                           // MessageBox.Show(spcode + "物料处于:" + s + ",请注意！" + s, "提醒", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {                      
                        ...
                        if (!dsprog.Tables[0].Rows[0]["Productcode"].ToString().StartsWith("7"))
                        {
                            ...
                            string productname = CheckString(dsprog.Tables[0].Rows[0]["productname"].ToString());
                            string vendorname = CheckString(ds.Tables[0].Rows[0]["vendorname"].ToString());
                            string sqlproxycard = "declare @pname varchar(max) set @pname='" + productname + "'";
                            ...
                            sqlproxycard += "  where productcode='" + ds.Tables[0].Rows[0]["materialcode"].ToString() + "' and supplier like '%" +vendorname+ "%' ";
                            ...
                            {
                              if (dsprog.Tables[0].Rows[0]["productname"].ToString().Contains("【F1"))
                              { 
                                string materialcode = CheckString(dsprog.Tables[0].Rows[0]["productname"].ToString());
                                string sqlproxycard1 = "declare @pname varchar(max) set @pname='" + materialcode + "'";
                                ...
                                if (dtproxycard1.Rows.Count > 0)
                                {

                                    for (int i = 0; i < dtproxycard1.Rows.Count; i++)
                                    {

                                        if ((Convert.ToDateTime(dtproxycard1.Rows[i]["expirydate"].ToString()) != Convert.ToDateTime("1900-01-01")) && (Convert.ToDateTime(dtproxycard1.Rows[i]["expirydate"].ToString()) != Convert.ToDateTime("0001-01-01")))
                                        {

                                            if (Convert.ToDateTime(dtproxycard1.Rows[i]["expirydate"].ToString()) < Convert.ToDateTime(dtproxycard1.Rows[i]["systime"].ToString()))
                                            {
                                                string info = "";
                                                lblinfo.Text = dsprog.Tables[0].Rows[0]["Productcode"].ToString() + "代理证有效期:" + dtproxycard1.Rows[i]["expirydate"].ToString() + ",代理证已超期!";
                                                info = lblinfo.Text;
                                                MessageBox.Show(info, "提醒", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                //lblinfo.ForeColor = Color.Red;
                                                //txtlotno.Focus();
                                                //txtlotno.SelectAll();
                                                //return;
                        ...
                        string materialsource = MaterialSource(dsprog.Tables[0].Rows[0]["Productcode"].ToString(),CheckString(ds.Tables[0].Rows[0]["vendorname"].ToString()));
                        string sup = "";
                        if (materialsource == "现货商")
                        {
                            sup = materialsource + ":" + ds.Tables[0].Rows[0]["vendorname"].ToString() + ",【频点:" + ds.Tables[0].Rows[0]["lot_number"].ToString() + "】" + "频率:" + pre;
                            lblsup.Text = sup;
                        ...
                        if (dsprog.Tables[0].Rows[0]["productname"].ToString().Contains("F1专用") || dsprog.Tables[0].Rows[0]["productname"].ToString().Contains("F1客供"))
                        {
                            ...
                            lblprodinfo.Text = "编码:" + dsprog.Tables[0].Rows[0]["Productcode"].ToString() + ",描述:" + productname(dsprog.Tables[0].Rows[0]["Productcode"].ToString().Trim());                
                        try
                        {
                        ...
                        lblinfoinstock.Text = MouldLife(ds.Tables[0].Rows[0]["materialcode"].ToString());
                        

                        //string stestitem = "select  t.TestType, t.TestItem, t.TestSubItem, t.TestDesc, t.TestTool, t.PackType, t.SampleType, t.UpValue, t.LowValue,t.UpScope,t.AQL, t.AQLValue,Samplevalue,Productcode,IFYiQi from  IQC_TestSubItemSet t inner join IQC_TestProgSet p on t.TestType=p.TestType and t.PackType=p.PackType ";
                        //stestitem += "  left join  IQC_TestSampleType s on t.SampleType=s.SampleType left join IQC_TestType ity on ity.TestType=t.TestTool where TTypes='测试工具' and Productcode='" + dsprog.Tables[0].Rows[0]["Productcode"].ToString() + "'";
            ...
            else
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ssp.WriteLine("FUNCtion:IMPedance R ");
                            DelayMs(100);
                            ssp.WriteLine("FUNC:IMP:RES:RANG:AUTO ON");
                            DelayMs(100);
                            ssp.WriteLine("APERture SLOW1");
                            DelayMs(100);
                            ssp.WriteLine("APERture:AVER 10 ");
                            DelayMs(100);
                            ssp.WriteLine("TRIGger:SOURce INTernal");
                            DelayMs(100);
                            ssp.WriteLine("DISPlay:PAGE MEASurement");
                            DelayMs(100);
                            ssp.WriteLine("FETC:AUTO ON ");
                        }
                        catch { }
                    }
                }
        ...
        private void btnElect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string res = "";
                read(addres, ref res);
                //txttestvalue.Text = res;

                //string res="+9.56897E-000,";
        ...
        private bool IfInputCorrect(string tval)
        {
            ...
            //20170515 add by husanbao
            else if (float.Parse(tval) < lowlittle || float.Parse(tval) > upbig)
            {
                txttestvalue.Text = tval.ToString();
                cbOK.Checked = false;
                cbOK.Enabled = false;
        ...
        string reciept_no(string lotno)
        {
            string reciept_id = "", row_id = "" ;
            string sql = @"  select RECIEPT_NO ,ROW_ID  from DIRECT_DELIVERY_RECIEPT_ITEMS  where  HYT_REELID = '"+ lotno + "'  ";
            DataTable dt = DbAccess.SelectBySql(sql).Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                reciept_id = dt.Rows[0]["RECIEPT_NO"].ToString();
                row_id = dt.Rows[0]["ROW_ID"].ToString();
            }
            return reciept_id;
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                if (lblunit.Text.ToUpper() != lblsetunit.Text.ToUpper() && cbOK.Checked)
                {
                    MessageBox.Show("测试单位与配置的单位不相同", "提醒", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return;
                }
            ...
            if (lotqty > 1 && txtsampletype.Text == "ISO2859-1")
            {
                int sampleqty = 0;
                if (!int.TryParse(txtsampleqty.Text.Trim(), out sampleqty))
                {           
                    return;
                }
                if (sampleqty == 1)
            ...
            string sstate = "NG";
            if (cbOK.Checked == false && cbNG.Checked == false && cbNA.Checked == false)
            {
            ...
            {
                sstate = "NG";
                if (cbNG.Enabled == true && (baddescribe == "" || NGtype == ""))
                {
            ...
            {
                string reciept_id = "", row_id = "";
                string sql = @"  select RECIEPT_NO ,ROW_ID  from DIRECT_DELIVERY_RECIEPT_ITEMS  where  HYT_REELID = '" + txtlotno.Text.Trim()+ "'  ";
                DataTable dt = DbAccess.SelectBydjiSql(sql).Tables[0];
                if (dt != null && dt.Rows.Count > 0)
                {
                    reciept_id = dt.Rows[0]["RECIEPT_NO"].ToString();
                    row_id = dt.Rows[0]["ROW_ID"].ToString();
                ...
                double i = rd.Next(1,10)*1.2;
                if (row_id == "")
                    row_id = txtlotno.Text.Trim();
                if (reciept_id == "")
                    reciept_id = "UMC_TEST";

                string djiaid = Guid.NewGuid().ToString();
                string TestResult = sstate == "OK" ? "合格接收" : "全部拒收";
                string djisql = "";
                djisql += " insert into IQC_TEST_LIST (ID ,INSPECTION_LOTNUMBER,MATERIALS_ROW_ID,MATERIAL_DETAILS ,OUT_SOURCING_VENDOR ,BATCH_NUMBERS ,";
                djisql += " UNQUALIFIED_QUANTITY ,SAMPLING_NUMBER ,SAMPLING_QUANTITY ,QUALIFIED_QUANTITY ,TEST_RESULT ,TEST_DATE ,";
			    djisql += " INSPECTORS,AUDIT_TIME ,AUDITOR ,EXCEPTION_DESCRIPTION )";
			    djisql += " values";
                djisql += " ('"+ djiaid + "','"+reciept_id+ "','"+row_id+ "','"+lblprodinfo.Text+ "','海能达通信有限公司',cast(" + Decimal.Parse(txtlotqty.Text)+ " as decimal(18, 4)),cast( 0 as decimal(18, 4)),cast(" + Decimal.Parse(txtsampleqty.Text)+ " as decimal(18, 4)),cast(" + Decimal.Parse(txtsamplefactqty.Text)+ " as decimal(18, 4))";
                djisql += ",cast(" + Decimal.Parse(txtlotqty.Text)+ " as decimal(18, 4)), '" + TestResult+ "', GETDATE(), '"+Login.username+ "',DATEADD(hour,"+i+ ",GETDATE()),'李传玲','" + Baddescribe.Text+ "')  ";   //// 
                try
                {
                    bool flag = DbAccess.ExecutedjiSql(djisql);
                }
                catch
                {
                }
            ...
            try
            {
                ...
                if (txttestitem.Text.Contains("模具寿命监控"))
                {
                   string mouldmessage = DateTime.Now.ToString("yyyy-MM-dd")+"来料数量：" +lotqty.ToString()+"；"+lblinfoinstock.Text;

                    msg = ic.AddNewTestListNew("新增", testtype, txttestitem.Text, txttestsubitem.Text, mouldmessage, txttesttools.Text, txtPacktype.Text, txtsampletype.Text, float.Parse(txtscope2.Text == "" ? "0" : txtscope2.Text),
                    float.Parse(txtscope1.Text == "" ? "0" : txtscope1.Text), float.Parse(AQLValue == "" ? "0" : AQLValue), txtlotno.Text.Trim(),lotqty, Login.username, int.Parse(txtsampleqty.Text), 1, sproductcode, txtAQL.Text, sstate, "", txtremarks.Text, 0, "",
                    "", sCheckType, txtDateCode.Text, txtMdate.Text, txtExpiryDate.Text, txtrsno.Text == null ? "" : txtrsno.Text, NGtype, baddescribe, manufacturer, 0, "");
                ...
                else
                {
                   ...
                   float.Parse(txtscope1.Text == "" ? "0" : txtscope1.Text), float.Parse(AQLValue == "" ? "0" : AQLValue), txtlotno.Text.Trim(),lotqty, Login.username, int.Parse(txtsampleqty.Text), 1, sproductcode, txtAQL.Text, sstate, "", txtremarks.Text, 0, txttestvalue.Text,
        ...
        */
        ...
        public DataTable IQCCheckLotnoResult(string lotno)
        {
            string sql = " declare @recept varchar(30),@p varchar(30)";
            sql += "select @recept=receptid,@p=Productcode from IQC_TestList where LotNo='" + lotno + "'";
            sql += " SELECT  deliveryid,item, materialcode,d.lotno,(d.qty-isnull(finOK.qty,0)-isnull(finNG.qty,0)) qty,finOK.qty OKqty,finNG.qty NGqty,org_id,iqcTransactionid,'' TestResult,(d.qty-isnull(finOK.qty,0)-isnull(finNG.qty,0)) checkqty ,'' remark  from delivery d left join";
        ...
        {
            string states = "OK";
            if (!cbOK2.Checked && !cbNG2.Checked)
            {
            ...
            //if (txtMaterialState.Text == "")
            //{
            //    MessageBox.Show("请选择入库状态","提醒",MessageBoxButtons.OK,MessageBoxIcon.Information);
            //    return;
            //}
            if (lblprodinfo.Text.ToUpper().Contains("D1专用") || lblprodinfo.Text.ToUpper().Contains("D1客供") || lblprodinfo.Text.ToUpper().Contains("D1R专用") || lblprodinfo.Text.ToUpper().Contains("D1R客供"))
            {
            ...
            if (dtorg.Rows[0][0].ToString() == "HCL")
            {
                ...
                TestResultCheck TRC = new TestResultCheck(int.Parse(txtstockqty.Text), states, txtremarks.Text,lblprodinfo.Text, dtToERP);
            ...
            else if (dtEMS != null && dtEMS.Rows.Count > 0)
            {
                ...
                // DataTable dtEMSCusLotno = IQCCheckEMSCusLotnoResult(txtlotno.Text);
                DataTable dtEMSCusLotno = IQCCheckEMSCusLotno(txtlotno.Text.Trim());
                CheckEMSCusLotno TRC = new CheckEMSCusLotno( int.Parse(txtstockqty.Text),states,txtlotno.Text.Trim(),txtremarks.Text,lblprodinfo.Text, dtEMS,dtEMSCusLotno);
                TRC.ShowDialog();
                txtmadecode.Text = "";
                txtstockqty.Text = "";
               ///// txtMaterialState.Text = "";
                return;
               ...
               /*
                 ...
                 {
                     IQC ic = new IQC();
                     string barcodemsg = ic.IQC_TestOtherInstock(org_id, item_id, productcode, srecept, productcode, stockqty.ToString(), enableqty, Login.username, txtlotno.Text, states, txtremarks.Text);
                     if (barcodemsg.IndexOf("成功") >= 0)
                     {
                         lblinfoinstock.ForeColor = Color.Blue;
                         lblinfoinstock.Text = barcodemsg;
            ...
            {
                DataTable dtEMSLotno = IQCCheckLotnoResult(txtlotno.Text);
                CheckEMSLotno TRC = new CheckEMSLotno(int.Parse(txtstockqty.Text.Trim()), txtlotno.Text.Trim(), txtremarks.Text, states,lblprodinfo.Text, dt, dtEMSLotno);
                TRC.ShowDialog();
                txtmadecode.Text = "";
                txtstockqty.Text = "";
                return;
